...
function combatMisc() {
	...
	if(
		...
		if(useScare) this.scare().catch(console.error);
	}
}
...
function priest(boundary) {
	...
	if(!this.isOnCooldown('darkblessing') && this.mp > 1500) this.darkBlessing().catch(console.error)
	...
	if(this.hp < this.max_hp * 0.6) {
		this.healSkill(this.name).catch(console.error);
		return;
	}
	...
	) this.absorbSins(target.target).catch(console.error);

	if(this.mp > 700 && !this.isOnCooldown('curse')) this.curse(target.id).catch(console.error);
...
function warrior(priestName) {
	...
	const tauntStrategy = () => {
		...
		if(aggro && !this.isOnCooldown('taunt')) this.taunt(aggro.id).catch(console.error);
	}

	tauntStrategy();
	if(!this.isOnCooldown('warcry')) this.warcry().catch(console.error);
	...
	if(distanceToTarget > 200 && !this.isOnCooldown('charge')) this.charge().catch(console.error)
	...
	if(distance(this, target) > this.range * 0.2) {
		...
		if(!pathClear) {
			if(!this.isSmartmoving) {
				...
				.catch( () => this.isSmartmoving = false )
			} else {
				return;
			}
		} else {
